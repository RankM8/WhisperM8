...
- **QA 2026-09-23 in der App (Slice 1–3, nach `make dev`):** Kontoliste zeigt beide Konten; `ai` aktivieren startet die Instanz auf 18775 (Elternprozess WhisperM8, `CCP_CONFIG_DIR=~/.gpt-profiles/ai`); neuer GPT-Chat antwortet über ai@ (Proxy-Log 200 in 6,8 s); Kontextmenü „GPT-Konto" funktioniert. Befund: `main` zeigte „Limits nicht abrufbar — Token in der Datei veraltet", obwohl der manuelle main-Proxy (PID 90457, seit 08.09.) Prompts bedient — der Datei-Token ist seit dem 16.09. abgelaufen. **Fork-Quellcode (Tag v0.1.36-whisperm8.1, Auth-Code identisch mit HEAD) gelesen:** `use_macos_keychain()` ist nur OHNE `CCP_CONFIG_DIR` wahr; der Keychain-Write bailt im nicht-interaktiven Modus immer, `save` fällt dann auf die Datei zurück, `load` liest ohnehin die Datei zuerst. Laut Quelltext müsste ein Refresh die Datei also aktualisieren — die Datei blieb trotzdem auf dem 06.09. Widerspruch **ungeklärt** (kein Refresh-Logging im Proxy). Konsequenz unabhängig davon: WhisperM8 startet main jetzt ebenfalls mit `CCP_CONFIG_DIR` auf dem Default-Store (reiner Datei-Modus, derselbe Pfad), Login/Status für main ebenso; ein „Neu anmelden…" für main schreibt eine frische Datei. Empirischer Check offen: ändert sich `~/.gpt-profiles/ai/codex/auth.json` um den 03.10. (Ablauf des ai@-Tokens), persistiert der Fork Refreshs im Datei-Modus. Statuszeile zeigt seit 23.09. das GPT-Konto (`⇄gpt:ai` bzw. dezent `⇄gpt:main`), weil „main" dort sonst als Git-Branch und Claude-Konto doppelt gelesen wurde.
