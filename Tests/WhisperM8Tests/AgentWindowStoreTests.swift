...
final class AgentWindowStoreTests: XCTestCase {
    ...
    func testSetCompactStoresAndClearsExpandedFrame() {
        let store = makeStore()
        let w = store.primaryWindowID
        let frame = AgentWindowFrame(x: 10, y: 20, width: 900, height: 700)

        store.setCompact(true, in: w, expandedFrame: frame)
        XCTAssertTrue(store.isCompact(in: w))
        XCTAssertEqual(store.expandedFrame(in: w), frame)

        // Erneutes Verkleinern ohne Frame behaelt den gemerkten.
        store.setCompact(true, in: w)
        XCTAssertEqual(store.expandedFrame(in: w), frame)

        store.setCompact(false, in: w)
        XCTAssertFalse(store.isCompact(in: w))
        XCTAssertNil(store.expandedFrame(in: w), "Expand leert den gemerkten Frame")
    }

    func testCompactSurvivesTabMutations() {
        let store = makeStore()
        let w = store.primaryWindowID
        store.setCompact(true, in: w, expandedFrame: AgentWindowFrame(x: 0, y: 0, width: 1100, height: 720))
        let a = UUID(); let b = UUID()
        store.openTab(a, in: w)
        store.openTab(b, in: w)
        store.closeTab(a, in: w)
        XCTAssertTrue(store.isCompact(in: w),
                      "Tab-Mutationen (upsertWindow/normalizedWindows) verlieren den Kompakt-Zustand nicht")
        XCTAssertNotNil(store.expandedFrame(in: w))
    }

    func testDetachedWindowStartsExpandedSourceStaysCompact() {
        let store = makeStore()
        let primary = store.primaryWindowID
        let a = UUID(); let b = UUID()
        store.openTab(a, in: primary)
        store.openTab(b, in: primary)
        store.setCompact(true, in: primary, expandedFrame: AgentWindowFrame(x: 5, y: 5, width: 1000, height: 650))

        let secondary = store.detachToNewWindow(b, from: primary)
        XCTAssertFalse(store.isCompact(in: secondary), "neues Fenster startet gross")
        XCTAssertTrue(store.isCompact(in: primary), "Quell-Fenster bleibt kompakt")
    }

    func testSetCompactOnUnknownWindowIsNoOp() {
        let store = makeStore()
        let ghost = UUID()
        store.setCompact(true, in: ghost)
        XCTAssertFalse(store.isCompact(in: ghost), "kein Create-on-mutate fuer Geisterfenster")
        XCTAssertFalse(store.state.windows.contains { $0.id == ghost })
    }

    func testCompactStatePersistsAcrossReload() {
        let wsURL = tempURL("ws-compact")
        let uiURL = tempURL("ui-compact")
        let store = AgentWindowStore(
            persistence: AgentSessionStore(fileURL: wsURL, uiStateFileURL: uiURL)
        )
        let w = store.primaryWindowID
        store.setCompact(true, in: w, expandedFrame: AgentWindowFrame(x: 1, y: 2, width: 800, height: 600))
        store.flush()

        let reloaded = AgentWindowStore(
            persistence: AgentSessionStore(fileURL: wsURL, uiStateFileURL: uiURL)
        )
        XCTAssertTrue(reloaded.isCompact(in: w), "Kompakt-Zustand ueberlebt den Neustart")
        XCTAssertEqual(reloaded.expandedFrame(in: w)?.width, 800)
    }
