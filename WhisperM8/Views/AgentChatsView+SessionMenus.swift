...
extension AgentChatsView {
    ...
    func sessionContextMenu(
        ...
        if plan.showsAutoTitle {
            // Seit 2026-09-23 kein Modellaufruf mehr: der Titel kommt nativ
            // vom CLI (Claudes /rename bzw. generierter Titel, sonst erster
            // Prompt). Ein WhisperM8-Name lässt sich hier wieder aufheben.
            if session.titleIsAutoGenerated == false {
                Button("Benennung aufheben", systemImage: "arrow.uturn.backward") {
                    resetSessionName(session)
                }
                .disabled(session.externalSessionID == nil)
            } else {
                Button("Titel neu einlesen", systemImage: "arrow.clockwise") {
