...
struct AgentChatsView: View {
    ...
    /// Hebt einen WhisperM8-Namen auf und liest sofort den nativen Titel ein.
    func resetSessionName(_ session: AgentChatSession) {
        do {
            try store.resetSessionTitle(id: session.id)
        } catch {
            errorMessage = error.localizedDescription
            return
        }
        var reset = session
        reset.title = "\(session.provider.displayName) Chat"
        reset.titleIsAutoGenerated = true
        forceAutoNameSession(reset)
    }
