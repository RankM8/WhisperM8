...
struct AgentSessionStore {
    ...
    /// Hebt einen in WhisperM8 vergebenen Namen auf: Die Session trägt wieder
    /// den Provider-Default und gilt als automatisch benannt — der native Titel
    /// des CLI (`NativeSessionTitle`) übernimmt beim nächsten Einlesen.
    func resetSessionTitle(id: UUID) throws {
        try updateSession(id: id) { session in
            session.title = "\(session.provider.displayName) Chat"
