# CLI-Spezialpfad: Codex-Subagents via `whisperm8 agent`

> **Nur bei explizitem CLI-Bedarf laden.** Standard ist der native Agent-Typ
> `gpt` (SKILL.md). Diese Referenz gilt für detachte, App-sichtbare Jobs,
> Browser-QA mit Playwright-State, `image_gen`, Jobverwaltung und ausdrücklich
> gewünschte CLI-Steps in Dynamic Workflows.
...
8. **Committen ja, Pushen nein.** Anders als native GPT-Subagents (die nie
   committen — Gotcha „Kein Commit durch GPT-Agents" in SKILL.md) dürfen
   explizit beauftragte CLI-Jobs committen: das CLI schaltet bei
   workspace-write das gemeinsame Git-Verzeichnis automatisch frei
   (`git rev-parse --git-common-dir`; Codex' Sandbox behandelt es sonst als
   read-only) — Commits funktionieren in-place, aus Repo-Unterverzeichnissen
   und mit `--worktree`. Push nur nach User-Freigabe via `--allow-network`.
...
WhisperM8 vorhanden; in Fremd-Projekten wie akquise-ai fehlen sie). Vor dem Bau
...
> **Quoting-Regel (siehe Troubleshooting):** Typografische Anführungszeichen
> („…") und andere Sonderzeichen im Prompt brechen die
> zsh-Doppelquote-Umklammerung. Nur kurze Prompts ohne solche Zeichen inline
> übergeben; alles andere per Write in eine Datei legen und mit
> `"$(cat /pfad/prompt.txt)"` übergeben — wie in Vorlage 2.
...
#    Prompt vorher per Write nach prompt.txt: Auftrag, Testbefehl
