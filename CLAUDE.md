...
- **Foreground chats**: SwiftTerm `LocalProcessTerminalView` PTYs, managed by `AgentTerminalRegistry` (in `Views/AgentTerminalView.swift`). `AgentCommandBuilder` builds argv (claude/codex, resume vs. new session, keyboard profile per TUI type). **Link-Klicks** (Cmd-Klick; Cmd+Alt = im Finder zeigen): SwiftTerm reicht `requestOpenLink` NICHT an den `processDelegate` weiter, und der Extension-Default (`URL(string:)+NSWorkspace.open`) scheitert bei schemelosen Dateipfaden mit `-50`. Da `requestOpenLink` auch nicht `open`/überschreibbar ist, ersetzt `AgentTerminalLinkInterceptor` den `terminalDelegate` (proxied alle Callbacks 1:1 an die Basis — so bleibt z. B. der Scroll-Lock erhalten — und fängt nur Links ab) und routet über den puren, getesteten `TerminalLinkResolver` (Views/): Web→Browser, Code/Markdown→PhpStorm (`PhpStormLauncher`, Services/Shared), sonstige Datei→Standard-App, Ordner→Finder, fehlend→klare `NSAlert`; relative Pfade gegen `command.workingDirectory`, `~` und `path:line`-Suffixe aufgelöst.
