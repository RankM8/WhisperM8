...
- **GPT-Backend, Proxy-Binary-Auswahl** (`ClaudeCodeProxyManager.resolvedBinary()`): Der Upstream-Proxy `claude-code-proxy` führt seine Modell-Allowlist einkompiliert; nur der WhisperM8-Fork (`GiulianoCosta71/claude-code-proxy`, Managed Download nach `~/Library/Application Support/WhisperM8/bin/`) liest `~/.codex/models_cache.json` pro Request. Ein PATH-Binary (Homebrew) hat deshalb nur noch Vorrang, wenn seine `--version` die Katalog-Allowlist beherrscht (`ClaudeCodeProxyBinaryInstaller.supportsCatalogAllowlist`, Fork ab 0.1.36-whisperm8.1; Upstream erst, wenn `minimumUpstreamCatalogVersion` gesetzt ist, sobald raine#130 released ist). Sonst gewinnt das verwaltete Binary, und `ensureRunning` installiert es einmal pro App-Lauf selbst; scheitert das, läuft der alte Proxy mit Warnung weiter. Vorfall 2026-09-08: Homebrew 0.1.21 vom Juli gewann gegen alles, Router und `gpt.md` boten aus dem Katalog längst `gpt-6-astra` an, jeder `gpt`-Spawn starb mit „Unknown model". Alle drei Stellen lesen jetzt dieselbe Quelle: Router-Validierung (stat-gecacht pro Request), Fork-Proxy (mtime/len pro Request) und `gpt.md` (`CodexModelCatalogWatcher` stößt den Sync bei Katalog-Änderung an, zusätzlich zu jedem Backend-Start).
