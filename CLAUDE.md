...
- **Terminal-Scrollback** (`TerminalScrollbackPolicy`, Default 5000 Zeilen statt SwiftTerms 500): Ist der Ringpuffer voll — bei jedem länger laufenden Chat also immer —, senkt jede neue Ausgabezeile `yDisp`, die Leseposition des Nutzers, um genau 1. Das ist SwiftTerms korrekte Kompensation (der Text bleibt stehen statt zu springen), läuft aber gegen 0, und bei 0 ist der obere Anschlag erreicht. Mit 500 Zeilen war ein um 300 Zeilen hochgescrollter Nutzer nach nur ~200 Zeilen Agent-Output oben angeschlagen — eine einzige Antwort mit Tool-Ausgaben reicht dafür. Genau das war der Unterschied zwischen „normalen" Chats (man liest, während der Agent idle ist: kein Output, kein Trimmen) und Background-Agents/Agents-View (Dauer-Output genau während man liest). Der Default ist aus Nutzen gegen Speicher gewählt (worst case, Puffer voll beschrieben: 8 Grid-Panes 85 MB, ein Vollbild-Chat 28 MB; 10000 Zeilen hätten 167 MB gekostet), anpassbar per `defaults write com.whisperm8.app agentTerminalScrollbackLines -int 20000`. Preis: der Scrollbar-Knob wird winzig und liegt auf SwiftTerms 1-%-Untergrenze. **Ein deaktivierter `NSScroller` zeichnet den Track ohne Knob** — seit dem `.legacy`-Stil blieb dadurch eine leere Rinne stehen, die Scrollbarkeit vortäuscht; `syncScrollerVisibility()` blendet ihn bei `!canScroll` aus. Nicht verwechseln mit `TerminalScrollGuard`: dessen Alt-Buffer-Pfad ist seit Claude Code 2.1.220 **inaktiv** (gemessen: weder `claude`, noch `claude agents`, noch `codex` schalten Alternate-Screen oder Mouse-Tracking ein).
